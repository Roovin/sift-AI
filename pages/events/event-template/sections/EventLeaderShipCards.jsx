...
import Styles from '../../../../styles/about/leadershipCards.module.css';
...
export default function EventLeadershipCard(props) {
	...
	return (
		<>
		<section className={`LeadershipCards bg-darkBlue h-full relative`} id="leadership">
			...
			{data?.title && <div className={`w-full text-white mb-[25px] lg-up:pr-[20px] xl:mb-[24px] laptop-landscape:mb-[13px]`}>
				<h2>{data?.title}</h2>
			</div>}
			{data?.blurb &&<p className={` pr-[65px] text-white md:p-0 laptop-landscape:pr-[98px]`}>
				{data?.blurb}
			</p>}
			</div>
				<div className="wrap">
					<div className="cards-wrap flex flex-wrap w-[calc(100%+40px)] ml-[-20px]">
						{data?.speakers && data?.speakers.map((card, index) => {
							return (
								<div key={index}
									data-aos="fade-up"
									data-aos-delay="200"
									...
									<div className="content p-[32px] pb-[48px] h-full md:pb-[60px]">
										...
										{card?.socialDetails && card?.socialDetails.map((info, i) => {									
											return(<Link key={i} className={`mr-[24px] ${Styles.logoWrap}`} href={info?.url} target="_blank">
											{info?.type == 'linkedin' ? (
												<svg
												width="24"
												height="25"
        										...
        										<svg
													...
													fill="none"
													xmlns="http://www.w3.org/2000/svg" >
													<path d="M17.6074 3.43555H20.5975L14.065 11.0602L21.75 21.4355H15.7327L11.0197 15.1429L5.627 21.4355H2.63506L9.62227 13.2802L2.25 3.43555H8.42007L12.6802 9.18724L17.6074 3.43555ZM16.558 19.6079H18.2148L7.51978 5.16724H5.7418L16.558 19.6079Z" fill="white"/>
												</svg>
      										)}
										...
										<div className="ctaWrap absolute bottom-[32px] left-[32px] cursor-pointer">
											...
											>
												...
												{card?.ctaLabel ? card?.ctaLabel : 'Read Bio'}
											</div>
										</div>
									</div>
								</div>
		...
		{overlay === true ? (
				<div className={`overlay LeadershipCard transition-all duration-300 xl-up:w-[calc(100%-276px)] desktop:w-[calc(100%-244px)] wide-nav-screen:max-w-[2500px] wide-nav-screen:mx-auto  fixed w-screen h-screen z-[9999] ${Styles.overlay}`}>
					<div
						className={`${Styles.overlaycardsWrap} relative top-[50%] tablet-landscape:top-0 md:top-0 lg:pb-[50px] pt-[70px]`}
					>
						<div
							className={`${Styles.cross} w-[40px] h-[40px] cursor-pointer absolute right-[179px] top-[35px] xxl-up:right-[215px] lg:right-[15px] lg:top-[40px] wide-screen:right-[385px] ${Styles.logoWrap} z-[9999]`}
							...
							{data?.speakers && data?.speakers.map((card, index) => {
								return (
									<div key={index} className="py-[10px] md:pb-0">
										<div
											className={`${Styles.cardWrap} group xl:py-[20px] px-[70px] py-[40px] tablet-landscape:pb-[60px] xxl-up:min-h-[460px] bg-skyblue mx-[16px] flex md:flex-wrap relative card w-[calc(100%-28px)] h-full transition-all duration-[0.3s] my-[20px] border-[2px] border-white rounded-[16px] overflow-hidden`}
										>
											<div className="colTwo md:h-full h-full w-[38%] xxl-up:w-fit pr-[14px] xxl-up:pb-[45px] md:w-full md:pr-0">
												...
												<div className="shareLinks flex justify-center max-w-[297px] w-full mx-auto">
													{card?.socialInfo && card?.socialInfo.map((info, i) => {									
														return(
														<Link key={i} className={`mr-[32px] w-[30px] h-[30px] last:mr-0 ${Styles.logoWrap}`} href={info?.source} target="_blank">
														{info.socialType == 'linkedin' ? (
																<svg
																width="30"
																height="30"
																viewBox="0 0 24 25"
											...
											<div className="colTwo w-[62%]  xxl-up:w-fit pl-[10px] xxl-up:pl-[30px] md:w-full md:mt-[10px] md:pl-0">
												<div className="content pb-[80px] md:pb-0 h-full">
													<div className="text-black mb-[30px] md:mb-0 max-w-[95%] md:text-center">
														<h3 className=" mb-[4px]">{card.name}</h3>
														<span className=" text-[16px] mb-[8px] font-[700] uppercase tracking-[1.65px]">
															{card?.designation}
														</span>
														<div className=" mt-[30px] xxl:text-[14px] md:mt-[10px]" dangerouslySetInnerHTML={{ __html: card?.bio }}/>
													</div>
												</div>
											</div>
										</div>
						...
						<div className="flex items-center justify-center mt-[0] ml-[8px] xxl:pb-[35px] z-[10]">
							<button className={` mr-[30px] ${Styles.prevBtn}`} onClick={handlePrev}>
								<svg
									width="40"
									height="40"
									viewBox="0 0 40 40"
