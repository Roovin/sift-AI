...
export default function FullWidthMedia({ intro , bg }) {
	...
	return (
		<section className={`FullWidthMedia ${bg === "skyblue" && "bg-skyblue"} ${bg === "white" && "bg-white"}  pb-0`} id='about-us'>
			<div className="container">
				{(intro.cta || intro.cta2) && <IntroWithCta {...intro} />}
				{!intro.cta && <FullWidthIntro {...intro} />}
				<div
					className={`${Style.contentWrapper} laptop:mt-[60px] laptop-landscape:mt-[40px!important] xl:mt-0 mt-[80px] w-full h-full sm:h-[300px] rounded-[16px] overflow-hidden relative cursor-pointer `}
				...
				>
					<div className={`${Style.posterWrap} sm:h-[300px]`}>
						<Image
							...
							width={1164}
							height={502}
							src="/VideoWithContent/videoCover.png"
							alt="Image"
						/>
					</div>
					<div
						className={`${Style.playbtn} absolute top-[50%] left-[50%] w-[39px] h-[39px] z-[3]`}
					>
			...
			{videoOverlay === true ? (
				<div
					className={`overlay transition-all duration-300 xl-up:w-[calc(100%-276px)] desktop:w-[calc(100%-244px)] wide-nav-screen:max-w-[2500px] wide-nav-screen:mx-auto  fixed w-screen h-screen z-[9999] ${Style.overlay}`}
				>
					...
					<div className={`cross w-[40px] h-[40px] cursor-pointer absolute right-[179px] -top-[27px] desktop-laptop:right-[30px] desktop-laptop:top-[45px] lg:right-[30px] lg:top-[40px]  ${Style.logoWrap} z-[9999]`} onClick={() => removeOverlay()}>
						...
						<div className="videoWrap w-full max-w-[760px] mx-auto p-[20px]">
								<iframe
										width="100%"
										height="415"
										src="https://www.youtube.com/embed/NTZcKmgWmN0?si=6myFNKgSNHFOAptI?mute=1&autoplay=1"
