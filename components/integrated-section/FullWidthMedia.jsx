...
export default function FullWidthMedia(props) {
	...
	const clickOverlay = () => {
		setVideoOverlay((prevVideoOverlay) => !prevVideoOverlay);
	};
	const removeOverlay = () => {
		setVideoOverlay((prevVideoOverlay) => !prevVideoOverlay);
	};
	...
	if(data?.videos !== null){
		 ...
		 match = data?.videos[0]?.videoUrl.match(regExp);
		videoId = (match && match[5].length === 11) ? match[5] : 'error';
		if ((data?.videos[0]?.videoUrl)?.includes('vimeo')) {
			iframeSrc = data?.videos[0]?.videoUrl
		  } else {
	...
	return (
		<section id={data.id ? data.id : ''} className={`${data.gatedComponent? 'GatedHide' : ''} FullWidthMedia ${data.background} ${data.classes} ${data.padding} ${data.gatedComponent? 'GatedHide' : ''}`}>
			<div className="container">
				{data?.typeOfMedia == 'image' ?
				<div className="  w-full h-full sm:h-[300px] rounded-[16px] overflow-hidden relative">
					{data?.image && data?.image?.sourceUrl &&
						<div className=" sm:h-[300px]">
							<Image
								className="w-full h-full relative z-[1] sm:object-cover"
								width={1164}
								height={502}
				...
				: 
				<div className={`${Style.contentWrapper}  w-full h-full sm:h-[300px] rounded-[16px] overflow-hidden relative cursor-pointer`} onClick={() => clickOverlay()}>
					{data?.videos && data?.videos[0]?.posterImage?.sourceUrl &&
						<div className={`${Style.posterWrap} sm:h-[300px]`}>
							<Image
								className="w-full h-full relative z-[1] sm:object-cover"
								width={1164}
								height={502}
								src={data?.videos[0]?.posterImage?.sourceUrl}
								alt={data?.videos[0]?.posterImage?.altText}/>
						</div>
					}
					<div className={`${Style.playbtn} absolute top-[50%] left-[50%] w-[39px] h-[39px] z-[3]`}>
						<div className={Style.play}></div>
					</div>
				</div>
				}
			</div>
			{videoOverlay === true && data?.typeOfMedia == 'video' ? (
				<div className={`overlay transition-all duration-300 xl-up:w-[calc(100%-276px)] desktop:w-[calc(100%-244px)] wide-nav-screen:max-w-[2500px] wide-nav-screen:mx-auto  fixed w-screen h-screen z-[9999] ${Style.overlay}`}>
