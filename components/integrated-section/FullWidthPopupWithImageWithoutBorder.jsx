...
export default function FullWidthPopupWithImageWithoutBorder(props) {
	...
	return (
		<section className={`Popup md:pt-[140px]  md:pb-[140px] !fixed left-0 top-0 z-[99] overflow-y-scroll h-[100vh] ${style.popupPatterns} relative bg-black ${isRemovePopup === true ? '' : 'hidden'}`}>
			<div className="container top-[50%] transform  -translate-y-1/2 md:transform-none md:top-0">
				<div className={`popupWrap w-full relative max-w-[1105px] mx-auto `}>
					<div className={`imgWithContent`}>
						<div className={`${style.crossBtn} w-fit ml-auto cursor-pointer absolute top-0 sm:-top-[40px] right-0 laptop:right-[20px] xl:-top-[50px] xl:right-[20px] md:w-[25px]`} onClick={() => removePopup()}>
                        ...
                        {data?.image && data?.image?.sourceUrl && <div className="iconWrap mb-[16px] w-[45%] md:w-full bg-darkBlue rounded-[24px] overflow-hidden h-[450px] laptop:h-auto xl:h-auto">
								<Image
							...
							<div className="rightText w-[55%] md:w-full pl-[40px] sm:pl-0">
                                {data?.title && <h3 className="mb-[16px]">{data?.title}</h3>}
								{data?.blurb && <p>{data?.blurb}</p>}
								<div className="btn mt-[16px] text-black">
