...
export default function LeadershipCards(props) {
	...
	return (
		<>
			<section
				id={data.id ? data.id : ''}
				className={`LeadershipCards  h-full relative no-padding-top ${data.background} ${data.padding} ${data.classes}`}
			>
				{data?.background == 'custom' && (
					<div className="custom-bg-img absolute left-0 top-[0px] w-full h-full ">
						{data?.backgroundImage && data?.backgroundImage?.sourceUrl && (
							<Image
				...
				<div className="img-box absolute -bottom-[150px] right-0 w-[330px]  laptop:w-[275px] desktop:w-[275px] laptop-landscape:w-[245px!important] h-[585px] lg:hidden">
				...
				<div className="container z-[2]">
					<div className="wrap">
						<div className="links mb-[60px] lg:mb-[40px]">
							<div
						...
						<div className="cards-wrap flex flex-wrap w-[calc(100%+24px)] ml-[-12px]">
							{data &&
								data?.content[activeTab]?.cards &&
								data?.content[activeTab]?.cards.map((card, index) => {
									return (
										<div
											key={index}
											data-aos="fade-up"
											data-aos-delay="200"
											className="group bg-black relative card phablet:w-[calc(70%-24px)] sm:w-[calc(100%-24px)] phablet:mx-auto wide-screen:w-[calc(25%-24px)] xxl-up:w-[calc(25%-24px)] cursor-pointer tablet:w-[calc(50%-24px)] tablet-mid:w-[calc(50%-24px)] laptop-mid:w-[calc(33.33%-24px)] laptop:w-[calc(33.33%-24px)] w-[calc(25%-24px)] transition-all mx-[12px] duration-[0.3s] mb-[40px] lg:mb-[24px] border-[2px] border-white rounded-[16px] overflow-hidden shadow-none hover:shadow-white-shadow"
											onClick={() => clickOverlay(index)}
										>
											<div className="imgwrap h-[242px] md:h-[320px] sm:h-[250px] tablet:hidden md:hidden xxl-mid:h-[285px] xxl-mid-up:h-[415px]  wide-screen:h-[370px] tablet-mid:h-[380px] laptop-mid:h-[290px] laptop:h-[340px] w-full overflow-hidden transition-all duration-[0.3s]">
												{card?.posterImage?.sourceUrl && (
													<Image
														className="w-full object-cover  h-full scale-none group-hover:scale-[1.1] transition-all duration-[0.3s] "
														width={200}
														height={200}
														src={card?.posterImage?.sourceUrl}
														alt={card?.posterImage?.altText}
													/>
												)}
											</div>
											<div className="content p-[32px] pb-[80px] h-full md:pb-[60px]">
												...
												<div className="shareLinks flex">
													{card?.socialInfo &&
														card?.socialInfo.map((info, i) => {
															return (
																<Link
																	key={i}
																	className={`mr-[24px] ${Style.logoWrap}`}
																	href={info?.source ? info?.source : '#'}
																	target="_blank"
																>
														...
														{card?.socialInfo &&
															card?.socialInfo.map((info, i) => {
																return (
																	<Link
																		key={i}
																		className={`mr-[32px] w-[30px] h-[30px] last:mr-0 ${Style.logoWrap}`}
																		href={info?.source ? info?.source : '#'}
																		target="_blank"
																	>
																		{info?.socialType == 'linkedin' ? (
												...
												<div className="colTwo w-[62%]  xxl-up:w-fit pl-[10px] xxl-up:pl-[30px] md:w-full md:mt-[10px] md:pl-0">
													<div className="content pb-[80px] md:pb-0 h-full">
														<div className="text-black mb-[30px] md:mb-0 max-w-[95%] md:text-center">
															...
															<div
																className=" mt-[30px] xxl:text-[14px] md:mt-[10px]"
