...
function VideoButton(props) {
  ...
  const overlay = <div className={`overlay transition-all duration-300 xl-up:w-[calc(100%-276px)] desktop:w-[calc(100%-244px)] wide-nav-screen:max-w-[2500px] wide-nav-screen:mx-auto  fixed w-screen h-screen z-[9999] ${Style.overlay}`}>
  ...
  return (
    <>
      {(open == 1 ?(overlay):'')}
      <Link href={props?.link} aria-label="first link" className={`${Styles.btn_pink} ${Styles.video_btn} pr-[60px] button z-10 inline-block transition-all md:py-[12px] laptop-landscape:px-[20px] laptop-landscape:pr-[40px] md:px-[20px] md:pr-[40px] desktop:pr-[55px]  py-[16px] px-[24px] active:opacity-[0.85] font-[700]  border-[3px] border-transparent bg-pink rounded-full uppercase `} onClick={(e)=>clickHandler(e)}>{props.text}</Link>
    </>
  )
}
